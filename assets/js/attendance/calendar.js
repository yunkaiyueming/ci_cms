    ...
    for (i  in  sFtv)
        if (sFtv[i].match(/^(\d{2})(\d{2})([\s\*])(.+)$/))
            if (Number(RegExp.$1) == (m + 1)) {
                ...
                //alert(this[Number(RegExp.$2) - 1].solarFestival);
...
function mOvr(thisObj, v) {
    ...
    if (sObj.html() != '') {
        ...
        arr.push('<div id="teshu"><FONT  COLOR="#ff0000"  STYLE="font-size:1px;">' + cld[d].solarTerms + ' ' + cld[d].solarFestival + ' ' + cld[d].lunarFestival + '</FONT></div>');
...
function mOck(thisObj, v){
	var onoff = thisObj.attributes["on"].value;
        var gd = thisObj.attributes["id"].value;
	...
	dayJson = '{holiday:'+nian+addZ(yue)+addZ(day)+',lx:'+lx+','+gd+'}';
	if(onoff == '0'){
...
//    global.currMonth = Number(dateObj.getMonth()) - 1;
//    alert(global.currMonth);
//    dateSelection.init();
//
//}
function initRiliIndex(yer,mon) {
    global.currYear = yer;
    global.currMonth = mon;
    dateSelection.init();

    clear();
    $("#nian").html(yer);
    $("#yue").html(parseInt(mon) + 1);
